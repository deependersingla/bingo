class UpdateMatrix
  def self.update(game_hash)
    ...
    if user_lines_cut < game_hash[:level]
      ...
      user_matrix = find_comp_element(game_hash[:user_matrix], element)
  ...
  def self.find_comp_element(matrix, number)
    matrix.map! {|line| line.map! {|x| x == "Y"? 0:x}}
    matrix.map! {|line| line.map! {|x| x == (number.to_i)? "Y":x}}
  end
end